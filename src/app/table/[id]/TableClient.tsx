...
import { BeliefCorrectionForm } from "@/components/BeliefCorrectionForm";
...
export function TableClient({ tableId, you }: { tableId: string; you: string }) {
      ...
      <main>
        <section className="workspace">
          <p className="inlineHint">
            This table link doesn&apos;t exist. <Link href="/">Create a new table</Link>.
          </p>
        </section>
      </main>
    );
  ...
  const seated = table.seatedDinerIds.includes(you);
  ...
  const join = async () => {
    setJoining(true);
    await runAction(() => joinTableRequest(tableId));
    setJoining(false);
  };

  const correctBelief = (kind: BeliefKind, value: string | number, correctionText: string) =>
  ...
  return (
    <main>
      <header className="topbar">
        ...
        <button type="button" className="secondaryButton" onClick={signOutAndHome}>
          Sign out
        </button>
      </header>
      ...
      <section className="workspace">
        {!seated ? (
          <>
            ...
            <button type="button" className="primaryButton" onClick={join} disabled={joining}>
          ...
          <>
            ...
            <section className="panel" aria-labelledby="roster-title">
              <div className="panelHeading">
                ...
                <span>{table.seatedDinerIds.length} seated</span>
              </div>
            ...
            {table.revision && (
              <BeliefRevisionPanel
                ...
                diner={table.diners.find((d) => d.id === table.revision?.dinerId)}
              />
            )}
            ...
            {!table.approved && (
              <div className="actionRow">
                {!table.revision && <BeliefCorrectionForm busy={busy} onSubmit={correctBelief} />}
                <button
                  type="button"
