/**
 * Standalone verification of the real xtraceMemoryGateway against the same
 * acceptance checks as verify.ts (the mock). Makes real XTrace network calls.
 *
 *   MEMORY_PROVIDER=xtrace npm run verify:xtrace
 *
 * Requires XTRACE_API_KEY to be set in the environment.
 */
import { xtraceMemoryGateway as g } from "@/features/memory";
...
async function main() {
  if (!process.env.XTRACE_API_KEY) {
    console.error("XTRACE_API_KEY is not set. Aborting before making any network calls.");
    process.exit(1);
  }
  ...
  console.log(
    failures === 0
      ? "\nAll memory acceptance checks passed (against real XTrace)."
